...
import { AdvancedDataTableComponent, DataTableOptions } from '../../../theme/shared/components/advanced-datatable/advanced-datatable.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, AdvancedDataTableComponent, AdvancedSearchComponent],
})
export class Cheque31Component implements OnInit {
  fichiers: Fichier[] = [];
  ...
  loadFichiers(): void {
    this.fichierService.getFichiersByNatureAndCodeValeur('Chèque', '31').subscribe({
      next: (data: Fichier[]) => {
        this.fichiers = data;
  ...
  filterData() {
    ...
    if (this.searchNomFichier) {
      filteredData = filteredData.filter(f =>
        f.nomFichier.toLowerCase().includes(this.searchNomFichier.toLowerCase()));
    }
    ...
    if (this.searchDateCreation) {
      filteredData = filteredData.filter(f =>
        f.createdAt.includes(this.searchDateCreation));
    }
    if (this.searchCodeEN) {
      filteredData = filteredData.filter(f =>
        f.codEn.toLowerCase().includes(this.searchCodeEN.toLowerCase()));
    }
    if (this.searchCode30) {
      filteredData = filteredData.filter(f =>
        f.codeValeur.toLowerCase().includes(this.searchCode30.toLowerCase()));
    }
  ...
  private applyAdvancedFilters(filters: SearchFilters) {
    ...
    if (filters.nomFichier) {
      filteredData = filteredData.filter(f =>
        f.nomFichier.toLowerCase().includes(filters.nomFichier.toLowerCase()));
    }
