...
export class Virement10Component implements OnInit {
  ...
  loadFichiers(): void {
    this.fichierService.getFichiersByNatureAndCodeValeur('Virement', '10').subscribe({
      next: (data: Fichier[]) => {
        this.fichiers = data;
  ...
  buildDatatableOptions(data: Fichier[]): DataTableOptions {
    return {
      ...
      data: data.map((f) => ({
        ...f,
        // Adapter les noms à ce que le tableau attend
        COD_EN: f.codEn,
